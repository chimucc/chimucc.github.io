...
if (!function_exists('mget')) {
    function mget(){
        if (func_num_args() >= 1) {
            ...
            $key = $args[0];
            $default = @$args[1];
            return Pingeon_Settings::instance()->get($key, $default);
        }else {
            return Pingeon_Settings::instance();
        }
    }
...
if (!function_exists('mset')) {
    ...
    function mset(){
        if (func_num_args() >= 1) {
            ...
            if (count($args) == 1) {
                $value = $args[0];
                Pingeon_Settings::instance()->set($key, $value);
            }else{
                Pingeon_Settings::instance()->set($key, $args);
            }
        }
